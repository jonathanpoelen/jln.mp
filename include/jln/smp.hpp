...
{
  template<class Seq, class TC = listify, class FC = clear<TC>>
  using after = typename mp::conditional_c<emp::is_list_v<Seq>>
...
namespace jln::mp::smp
{
  template<class Seq, class SubC1 = listify, class SubC2 = SubC1,
           class TC = listify, class FC = clear<TC>>
  using before_after_with = typename mp::conditional_c<emp::is_list_v<Seq>>
    ::template f<
...
namespace jln::mp
{
  ...
  namespace emp
  {
    ...
    template<class L, class F, class FallbackValue, class C = mp::identity>
    using reverse_fold_right_or = unpack<L,
      mp::reverse_fold_right_or_else<F, always<FallbackValue>, C>>;

    template<class L, class F, class EmptyC = F, class C = mp::identity>
    using reverse_fold_right_or_else = unpack<L,
      mp::reverse_fold_right_or_else<F, EmptyC, C>>;
...
{
  template<class OffsetEnd, class Pred, class TC = listify, class FC = TC>
  using partial_take_while_xs = partial_take_while_extended_by_n_xs<OffsetEnd, number<0>, Pred, TC, FC>;

  template<class OffsetEnd, class Pred, class TC = listify, class FC = TC>
  using partial_take_inclusive_while_xs = partial_take_while_extended_by_n_xs<OffsetEnd, number<1>, Pred, TC, FC>;
}
  ...
  using set_intersection = test_contract<
    ...
    mp::if_<
      mp::pop_front<
        ...
        >
      >,
      mp::try_<mp::set_intersection<subcontract<C>>>,
      violation
    >
  >;
}
