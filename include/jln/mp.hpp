...
#if JLN_MP_HAS_MEMOIZED_ALIAS || (JLN_MP_CUDA && JLN_MP_HOST_COMPILER_GCC)
...
namespace jln::mp
{
...
#if ! JLN_MP_OPTIMIZED_ALIAS && ! JLN_MP_DEBUG
  template<template<class...> class F, class C>
  struct reverse_fold_right<cfe<F>, C>
  {
    template<class... xs>
    using f = JLN_MP_CALL_TRACE(C,
      typename detail::reverse_fold_right_impl<int(sizeof...(xs)) - 1>
  ...
  template<template<class...> class F>
  struct reverse_fold_right<cfe<F>, identity>
  {
    template<class... xs>
    using f = typename detail::reverse_fold_right_impl<int(sizeof...(xs)) - 1>
      ::template f<F, xs...>;
...
namespace jln::mp::detail
{
  ...
  struct mk_split_keep_separator_indices
  {
    static constexpr std::size_t result_len = (0 + ... + is_sep) * 2 + 1;
    ...
    static constexpr auto make()
    {
      ...
      for (bool has_sep : is_seps)
      {
        if (has_sep)
        {
          // isolate separator
          ++nb_group;
          a.elems[nb_group][0] = i; // set index
          a.elems[nb_group][1] = 1; // set size
          // new group
          ++nb_group;
          a.elems[nb_group][0] = i + 1; // set index of the group
        }
        else
          ++a.elems[nb_group][1]; // increment size of the group
        ++i;
