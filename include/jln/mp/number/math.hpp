...
namespace jln::mp
{
  ...
  namespace emp
  {
    ...
    inline constexpr int_t min_c_v<x, y> = y < x ? y : x;
    ...
    template<int_t x, int_t y>
    inline constexpr int_t max_c_v<x, y> = x < y ? y : x;
    ...
    inline constexpr int_t clamp_c_v<i, min, max> = i < min ? min : max < i ? max : i;
    ...
    inline constexpr int_t clamp_with_c_v<i, min, max, Cmp> =
    ...
    template<int_t i, int_t min, int_t max>
    inline constexpr int_t clamp_with_c_v<i, min, max> = i < min ? min : max < i ? max : i;
    ...
    template<int_t i>
    inline constexpr int_t abs_c_v<i> = (i < -i ? -i : i)
    ...
    template<class Base, class Exponent>
    inline constexpr int_t pow_v<Base, Exponent> = detail::pow_impl_v<Base::value, Exponent::value>;
    ...
    inline constexpr int_t pow_c_v<base, exponent> = detail::pow_impl_v<base, exponent>;
